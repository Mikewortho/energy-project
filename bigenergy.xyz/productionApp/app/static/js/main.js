...
function parseStateColorsAndUpdate() {
    // console.log(data);
    $.get("http://playingthefield.xyz/bigenergy/data/stateColours.txt")
    .done( function(data) {
        console.log(data);
        var stateColours = Papa.parse(data);
        printMap(stateColours)
        // Papa.parse(data, {
        //     download: true,
...
function printMap()
{
    ...
    $.get("http://playingthefield.xyz/bigenergy/data/stateColours.csv")
    .done( function(data) {
        console.log(data);
        var stateColours = Papa.parse(data);

        for (var i = 1; i < stateColours.data.length; i++) {
            myStyles[stateColours.data[i][0]] = {fill: workOutValues(stateColours.data[i][1])};
        }
